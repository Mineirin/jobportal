...
switch ($action) {
	...
	function doInsert(){
		...
		if ( $_POST['FNAME'] == "" OR $_POST['LNAME'] == ""
			...
			OR $_POST['TELNO'] == "") {
			$messageStats = false;
			message("Todos os campos são necessários!","error");
			...
			redirect("index.php?view=add");
				...
				if ($maxrow > 0) { 
					# code... 
					message("ID de funcionário já em uso!", "error");
					redirect("index.php?view=add");
				}else{
					...
					message("Novo funcionário criado com sucesso!", "success");
					redirect("index.php");
	...
	if(isset($_POST['save'])){
		...
		if ( $_POST['FNAME'] == "" OR $_POST['LNAME'] == ""
			...
			$age = date_diff(date_create($birthdate),date_create('today'))->y;
		 	if ($age < 18 ){
		       message("idade válida. 18 anos ou mais é permitido.", "error");
		       redirect("index.php?view=edit&id=".$_POST['EMPLOYEEID']);
				...
				message("O funcionário foi atualizado!", "success");
	...
	function doDelete(){
		...
		// }
			message("Funcionário(a) já excluído!","success");
			redirect('index.php');
		// }
