...
<?php
$cover=0;
$id = $_REQUEST['id'];mysqli_connect('localhost','root','') or die(mysqli_error());mysqli_select_db('naukri2jobs');$result = mysqli_query("select * from rw_both WHERE exp_id=$id") or die(mysqli_error());
$amount = array();
    	...
    	if($id != '-1')
    	{
			...
			<tr><td align='center' style='font-size:15px;'>"; $_SESSION['AMOUNT']=$amount;
				...
				<form name='frm3' method='post'>
						...
						<tr><td><hr /></td></tr>
						<tr><td align='center'><input type='submit' name='groth_pack' value='Buy Now' style='color:#FFFFFF; margin-left:20px;font-family:Arial, Helvetica, sans-serif; background-color:#1014C2; border-radius:10px 10px 10px 10px; width:120px; height:40px; font-size:18px;' ></td><tr>
						<tr><td><hr /></td></tr>
					</table>";
   	}
 ?>